...
function initializeAmara() {
  ...
  const tree = document.getElementById("tree-amara");
  ...
  function render() {
    ...
    state.skills.forEach(skill => {
      ...
      if (skill.skillDataId && skill.val < skill.max) {
        canAdd = character.canAddSkillPoint('amara', skill.id, skill.val, character.level);
      }
    ...
    if (typeof generateShareLink === "function" && typeof updateShareLinkDisplay === "function") {
      const shareLink = generateShareLink(character, "Nakayuda");
