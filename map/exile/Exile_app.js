...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('Exilemonsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('ExileNPCGener.txt');
    ...
    const npcRes = await fetch('ExileNPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('ExilePortalLocation.json');
    if (portalRes.ok) {
