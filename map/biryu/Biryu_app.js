...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('Biryumonsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('BiryuNPCGener.txt');
    ...
    const npcRes = await fetch('BiryuNPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('BiryuPortalLocation.json');
    if (portalRes.ok) {
