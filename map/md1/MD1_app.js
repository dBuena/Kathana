...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('MD1monsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('MD1NPCGener.txt');
    ...
    const npcRes = await fetch('MD1NPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('MD1PortalLocation.json');
    if (portalRes.ok) {
