...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('Pamirmonsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('PamirNPCGener.txt');
    ...
    const npcRes = await fetch('PamirNPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('PamirPortalLocation.json');
    if (portalRes.ok) {
