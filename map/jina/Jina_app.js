...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('Jinamonsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('JinaNPCGener.txt');
    ...
    const npcRes = await fetch('JinaNPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('JinaPortalLocation.json');
    if (portalRes.ok) {
