...
async function loadLocations() {
  try {
    // Monster names
    const nameRes = await fetch('MD2monsternames.json');
    if (nameRes.ok) {
    ...
    const genRes = await fetch('MD2NPCGener.txt');
    ...
    const npcRes = await fetch('MD2NPCLocation.json');
    if (npcRes.ok) {
    ...
    const portalRes = await fetch('MD2PortalLocation.json');
    if (portalRes.ok) {
